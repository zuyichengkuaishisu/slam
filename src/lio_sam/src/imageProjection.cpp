...
// Must match livox_ros_driver2 comm.h LivoxPointXyzrtlt (#pragma pack(1), 26 bytes).
// PCL's aligned struct breaks fromROSMsg() field offsets for this layout.
#pragma pack(push, 1)
struct LivoxPointXyzrtltPacked
{
    float x;
    float y;
    float z;
...
#pragma pack(pop)
static_assert(sizeof(LivoxPointXyzrtltPacked) == 26, "Livox point layout mismatch");
...
public:
    ...
    void imuHandler(const sensor_msgs::msg::Imu::SharedPtr imuMsg)
    {
        ...
        const double imuStamp = stamp2Sec(thisImu.header.stamp);

        std::lock_guard<std::mutex> lock1(imuLock);
        if (lastImuStampSec_ >= 0 && imuStamp + kTimeJumpTolerance < lastImuStampSec_) {
            RCLCPP_WARN(get_logger(),
                "Detected sim time jump back, clearing imageProjection queues");
            imuQueue.clear();
            odomQueue.clear();
            cloudQueue.clear();
        }
        lastImuStampSec_ = imuStamp;
    ...
    void moveFromLivoxPointCloud2(const sensor_msgs::msg::PointCloud2& msg,
                                  pcl::PointCloud<PointXYZIRT>& cloud)
    {
        if (msg.point_step < sizeof(LivoxPointXyzrtltPacked)) {
            RCLCPP_ERROR(get_logger(),
                "Livox PointCloud2 point_step=%u, expected>=%zu",
                msg.point_step, sizeof(LivoxPointXyzrtltPacked));
            return;
        }
        const size_t point_count = static_cast<size_t>(msg.width) * msg.height;
        // Livox driver stores absolute time (ns) in the timestamp field, same as header base_time.
        const double scan_base_ns = static_cast<double>(
            rclcpp::Time(msg.header.stamp).nanoseconds());
        cloud.clear();
        cloud.reserve(point_count);
        for (size_t i = 0; i < point_count; ++i) {
            LivoxPointXyzrtltPacked src;
            memcpy(&src, &msg.data[i * msg.point_step], sizeof(src));
            PointXYZIRT point;
            point.x = src.x;
            point.y = src.y;
            point.z = src.z;
            ...
            point.tag = src.tag;
            point.ring = src.line;
            // LIO-SAM expects per-point time offset (seconds) from scan start
            point.time = static_cast<float>((src.timestamp - scan_base_ns) * 1e-9);
    ...
    bool cachePointCloud(const sensor_msgs::msg::PointCloud2::SharedPtr& laserCloudMsg)
    {
        ...
        if (sensor == SensorType::LIVOX || sensor == SensorType::VELODYNE)
        {
            moveFromLivoxPointCloud2(currentCloudMsg, *laserCloudIn);
        }
        else
        {
        ...
        if (laserCloudIn->empty()) {
            RCLCPP_WARN_THROTTLE(get_logger(), *get_clock(), 2000,
                "Livox point cloud is empty after parsing");
            return false;
        }
        ...
        float scanDuration = laserCloudIn->points.back().time;
        if (laserCloudIn->points.size() > 1) {
            scanDuration -= laserCloudIn->points.front().time;
        }
        if (scanDuration < 0.f) {
            scanDuration = 0.f;
        }
        if (scanDuration > 1.f) {
            RCLCPP_WARN_THROTTLE(get_logger(), *get_clock(), 2000,
                "Abnormal Livox scan duration %.3fs, clamping to 0.1s", scanDuration);
            scanDuration = 0.1f;
        }
    ...
    bool deskewInfo()
    {
            ...
            stamp2Sec(imuQueue.back().header.stamp) < timeScanEnd)
        {
            RCLCPP_INFO_THROTTLE(get_logger(), *get_clock(), 2000,
                "Waiting for IMU data (queue=%zu, scan [%.3f, %.3f], imu [%.3f, %.3f]) ...",
                imuQueue.size(), timeScanCur, timeScanEnd,
                imuQueue.empty() ? -1.0 : stamp2Sec(imuQueue.front().header.stamp),
                imuQueue.empty() ? -1.0 : stamp2Sec(imuQueue.back().header.stamp));
            return false;
        }

        // 扫描时间窗内需至少 2 个 IMU 样本，避免去畸变在“未就绪/就绪”间闪烁
        {
            int imuInScan = 0;
            for (const auto& imu : imuQueue) {
                const double t = stamp2Sec(imu.header.stamp);
                if (t >= timeScanCur - 0.01 && t <= timeScanEnd + 0.01) {
                    ++imuInScan;
                }
            }
            if (imuInScan < 2) {
