...
#include <fstream>
#include <sstream>
static void trae_dbg_post(const char* hypothesisId, const char* location, const std::string& msg,
...
public:
    ...
    double timeLastProcessing_ = -1;
    double lastLaserCloudTime_ = -1;
    static constexpr double kTimeJumpTolerance = 0.1;
    ...
    mapOptimization(const rclcpp::NodeOptions & options) : ParamServer("lio_sam_mapOptimization", options)
    {
        ...
        auto saveMapService = [this](const std::shared_ptr<rmw_request_id_t> request_header, const std::shared_ptr<lio_sam::srv::SaveMap::Request> req, std::shared_ptr<lio_sam::srv::SaveMap::Response> res) -> void {
            ...
            if (cloudKeyPoses3D->empty() || cloudKeyPoses6D->empty()) {
                cout << "No keyframes available, skip saving map" << endl;
                res->success = false;
                return;
            }
            ...
            int unused = system((std::string("exec rm -rf ") + shellQuote(saveMapDirectory)).c_str());
            unused = system((std::string("mkdir -p ") + shellQuote(saveMapDirectory)).c_str());
        ...
        allocateMemory();
        // #region debug-point A:node-init
        trae_dbg_post("A", "mapOptmization.cpp:ctor", "mapOptimization node constructed");
        // #endregion
    }
    ...
    void resetForSimTimeJump()
    {
        RCLCPP_WARN(get_logger(), "Detected sim time jump back, resetting map optimization state");
        ...
        isam = new ISAM2(parameters);
        allocateMemory();
        timeLastProcessing_ = -1;
        lastLaserCloudTime_ = -1;
    }

    void resetForBackendFailure(const char* where, const std::exception& e)
    {
        RCLCPP_WARN(get_logger(),
            "Map optimization backend failed in %s at t=%.6f keyframes=%zu: %s. Resetting.",
            where, timeLaserInfoCur, cloudKeyPoses3D->size(), e.what());
        resetForSimTimeJump();
    }

    void laserCloudInfoHandler(const lio_sam::msg::CloudInfo::SharedPtr msgIn)
    {
        // #region debug-point B:cloud-info-rx
        {
            static int rx_count = 0;
            if (++rx_count <= 5 || rx_count % 50 == 0) {
                trae_dbg_post("B", "mapOptmization.cpp:laserCloudInfoHandler",
                    "received feature cloud_info",
                    "{\"rx_count\":" + std::to_string(rx_count) + "}");
            }
        }
        // #endregion
        // extract time stamp
        ...
        if (lastLaserCloudTime_ >= 0 &&
            timeLaserInfoCur + kTimeJumpTolerance < lastLaserCloudTime_) {
            resetForSimTimeJump();
        }
        lastLaserCloudTime_ = timeLaserInfoCur;
        ...
        if (timeLaserInfoCur - timeLastProcessing_ >= mappingProcessInterval)
        {
            ...
            // #region debug-point C:process-tick
            trae_dbg_post("C", "mapOptmization.cpp:process-tick", "mapping process tick",
                "{\"time_cur\":" + std::to_string(timeLaserInfoCur) + ",\"interval\":" +
                std::to_string(mappingProcessInterval) + "}");
            // #endregion
            ...
            const bool scan2mapOk = scan2MapOptimization();
            // #region debug-point D:after-scan2map
            trae_dbg_post("D", "mapOptmization.cpp:after-scan2map", "scan2MapOptimization done",
                "{\"corner_ds\":" + std::to_string(laserCloudCornerLastDS->size()) +
                ",\"surf_ds\":" + std::to_string(laserCloudSurfLastDS->size()) +
                ",\"keyframes\":" + std::to_string(cloudKeyPoses3D->size()) +
                ",\"ok\":" + std::to_string(scan2mapOk ? 1 : 0) + "}");
            // #endregion
            if (!scan2mapOk) {
                return;
            }
    ...
    void visualizeGlobalMapThread()
    {
        ...
        int unused = system((std::string("exec rm -rf ") + shellQuote(savePCDDirectory)).c_str());
        unused = system((std::string("mkdir -p ") + shellQuote(savePCDDirectory)).c_str());
    ...
    void saveKeyFramesAndFactor()
    {
        ...
        try {
            ...
            resetForBackendFailure("isam-update", e);
            return;
        }
        ...
        try {
            ...
            if (isamCurrentEstimate.empty()) {
                RCLCPP_WARN(get_logger(), "Map optimization estimate is empty, skip keyframe.");
                return;
            }
            latestEstimate = isamCurrentEstimate.at<Pose3>(isamCurrentEstimate.size()-1);
        } catch (const std::exception& e) {
            resetForBackendFailure("calculate-estimate", e);
            return;
        }
        // cout << "****************************************************" << endl;
        ...
        try {
            ...
            RCLCPP_WARN(get_logger(),
                "Failed to compute pose covariance at keyframe %zu: %s",
                cloudKeyPoses3D->size(), e.what());
            poseCovariance = Eigen::MatrixXd::Identity(6, 6) * poseCovThreshold;
        }
    ...
    void publishOdometry()
    {
        ...
        // #region debug-point E:odom-published
        {
            static int odom_count = 0;
            if (++odom_count <= 5 || odom_count % 50 == 0) {
                trae_dbg_post("E", "mapOptmization.cpp:publishOdometry", "odometry published",
                    "{\"odom_count\":" + std::to_string(odom_count) +
                    ",\"x\":" + std::to_string(transformTobeMapped[3]) +
                    ",\"y\":" + std::to_string(transformTobeMapped[4]) + "}");
            }
        }
        // #endregion
        ...
        // 实时配准帧：每帧都发，不依赖关键帧数量或订阅者数量
        {
            ...
            *cloudOut += *transformPointCloud(laserCloudSurfLastDS,    &thisPose6D);
            if (!cloudOut->empty()) {
                ...
                // #region debug-point E:cloud-reg-published
                {
                    static int reg_count = 0;
                    if (++reg_count <= 5 || reg_count % 50 == 0) {
                        trae_dbg_post("E", "mapOptmization.cpp:publishFrames",
                            "cloud_registered published",
                            "{\"reg_count\":" + std::to_string(reg_count) +
                            ",\"points\":" + std::to_string(cloudOut->size()) + "}");
                    }
                }
                // #endregion
            }
