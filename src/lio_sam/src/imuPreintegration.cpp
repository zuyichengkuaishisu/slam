...
class IMUPreintegration : public ParamServer {
   public:
    ...
    void resetParams() {
        lastImuT_imu = -1;
        lastImuT_opt = -1;
        lastCorrectionTime_ = -1;
        doneFirstOpt = false;
        systemInitialized = false;
    }

    bool isSimTimeJumpBack(double newTime, double lastTime) const {
        return lastTime >= 0 && newTime + kTimeJumpTolerance < lastTime;
    }

    void resetForSimTimeJump(const char* where) {
        RCLCPP_WARN(get_logger(),
            "Detected sim time jump back in %s, resetting IMU preintegration", where);
        imuQueOpt.clear();
        imuQueImu.clear();
        lastImuT_imu = -1;
        lastImuT_opt = -1;
    ...
    void resetForOptimizationFailure(const char* where, const std::exception& e,
                                     double correctionTime, bool degenerate)
    {
        RCLCPP_WARN(get_logger(),
            "IMU preintegration optimization failed in %s at t=%.6f key=%d degenerate=%d: %s. Resetting.",
            where, correctionTime, key, degenerate ? 1 : 0, e.what());
        imuQueOpt.clear();
        imuQueImu.clear();
    ...
    double safeDt(double imuTime, double lastTime) const {
        if (lastTime < 0) {
            return 1.0 / 500.0;
        }
        const double dt = imuTime - lastTime;
        return (dt > 1e-6) ? dt : (1.0 / 500.0);
    }
    ...
    void odometryHandler(const nav_msgs::msg::Odometry::SharedPtr odomMsg) {
        ...
        if (isSimTimeJumpBack(currentCorrectionTime, lastCorrectionTime_)) {
            resetForSimTimeJump("odometryHandler");
        }
        lastCorrectionTime_ = currentCorrectionTime;
        ...
        if (systemInitialized == false) {
            ...
            try {
                ...
                resetForOptimizationFailure("initialization", e, currentCorrectionTime, degenerate);
                return;
            }
            graphFactors.resize(0);
            graphValues.clear();
        ...
        if (key == 100) {
            try {
                ...
                resetForOptimizationFailure("fixed-lag-reset", e, currentCorrectionTime, degenerate);
                return;
            }
        }
        ...
        while (!imuQueOpt.empty()) {
            ...
            if (imuTime < currentCorrectionTime - delta_t) {
                if (isSimTimeJumpBack(imuTime, lastImuT_opt)) {
                    resetForSimTimeJump("odometryHandler/imuQueOpt");
                    return;
                }
                const double dt = safeDt(imuTime, lastImuT_opt);
        ...
        try {
            ...
            resetForOptimizationFailure("graph-update", e, currentCorrectionTime, degenerate);
            return;
        }
        ...
        if (!imuQueImu.empty()) {
            ...
            for (int i = 0; i < (int)imuQueImu.size(); ++i) {
                ...
                double imuTime = stamp2Sec(thisImu->header.stamp);
                if (isSimTimeJumpBack(imuTime, lastImuQT)) {
                    resetForSimTimeJump("odometryHandler/imuQueImu");
                    return;
                }
                const double dt = safeDt(imuTime, lastImuQT);
    ...
    void imuHandler(const sensor_msgs::msg::Imu::SharedPtr imu_raw) {
        ...
        if (isSimTimeJumpBack(imuTime, lastImuT_imu)) {
            resetForSimTimeJump("imuHandler");
        }
        ...
        const double dt = safeDt(imuTime, lastImuT_imu);
